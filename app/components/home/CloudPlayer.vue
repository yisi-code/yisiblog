<template>
  <section
      class="glass-panel interactive-card group relative flex h-full origin-center flex-col justify-between overflow-hidden rounded-3xl p-card-inset transition duration-150 hover:scale-[1.02]"
  >
    <div
        class="music-player-glow absolute -right-20 -top-20 h-48 w-48 rounded-full blur-[50px] transition-opacity duration-150"
    ...
    <template v-else>
      <div class="relative mt-stack-xs flex items-center gap-4">
        ...
        >
          ...
          <div
              class="music-disc-center absolute left-1/2 top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border shadow-inner backdrop-blur-sm"/>
        </div>
      ...
      <div class="w-full relative z-10 mt-stack-xs flex h-fit items-center justify-between text-left">
        <div class="flex w-fit flex-1 items-center justify-center overflow-hidden pr-action-inline transform-gpu">
          ...
          >
            {{ displayedLyric }}
            <span
                class="animate-cursor ml-inline-gap-2xs inline-block h-5 w-0.75 align-middle bg-current"
                :style="{ animationPlayState: music.isPlaying ? 'running' : 'paused' }"
            />
          </MusicTextButton>
        </div>
        ...
        <div class="flex w-12 shrink-0 justify-end">
          <svg
              class="music-note-icon music-note h-6 w-6 transition duration-150"
              :style="{ animationPlayState: music.isPlaying ? 'running' : 'paused' }"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
      ...
      <div class="w-full flex items-center justify-center mt-stack-xs gap-4 relative" @click.stop>
        ...
        <div class="flex-1 text-secondary flex flex-col justify-center gap-3 text-size-meta-label font-bold">
