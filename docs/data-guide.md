...
管理页面位于 `/admin/data`，用于在本地或自部署 Node 服务环境中维护项目数据。
...
- 查询、新增、编辑、删除全部类型记录
- 保存时更新 `app/data/source/records.json`
- `post`、`chatter`、`moment`、`about` 自动生成或更新 Markdown 文件
- `music` 自动生成或更新 LRC 文件
- 类型专属字段直接写入记录根层级
...
- 如果使用 `npm run generate` 部署到纯静态平台，线上无法通过管理页面直接写文件。建议本地维护后重新生成并部署。
