...
项目会在读取时自动补齐缺失的 `tags`，并根据 `type` 与 `id` 生成默认页面 `path`；正文文件位置以 `contentUrl` 指向的数据胶囊对象为准。

## 自动路径规则

| type | 页面路径 | 关联文件 |
| --- | --- | --- |
| `post` | `/posts/{id}` | 数据胶囊 `博文/{id}/{id}.md` |
| `chatter` | `/chatter/{id}` | 数据胶囊 `杂谈/{id}/{id}.md` |
| `moment` | `/moments` | 数据胶囊 `动态/{id}/{id}.md` |
| `about` | `/about` | 数据胶囊 `关于/about/about.md` |
...
同步时会在数据胶囊 `博文/{id}/` 下创建或更新对应 Markdown。
...
同步时会在数据胶囊 `杂谈/{id}/` 下创建或更新对应 Markdown。
...
同步时会在数据胶囊 `动态/{id}/` 下创建或更新对应 Markdown。
...
```json
{
  ...
  "description": "专辑或说明",
  "cover": "https://example.com/cover.jpg",
  "url": "https://s3.cstcloud.cn/your-bucket/音乐/song/song.m4a",
  "lrcUrl": "https://s3.cstcloud.cn/your-bucket/音乐/song/song.lrc",
  "artist": "歌手"
}
```
...
- Markdown 正文以 `contentUrl` 为准，新增或编辑时由管理页同步到数据胶囊对应目录。
